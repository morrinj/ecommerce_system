...
class AdminController {
    ...
    public function login(): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($result['success']) {
                ...
                if (!empty($result['force_change'])) {
                    $_SESSION['force_password_change'] = true;
                    flash('warning', 'Your password is over 90 days old. Please change it for security.');
                }
                redirect('admin');
            }
    ...
    public function adminCreate(): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $minLen = $role === 'superadmin' ? 12 : 10;
            $errors = validatePassword($password, $minLen, $data['email'] ?? '', $data['full_name'] ?? '');
            if (!empty($errors)) {
    ...
    public function adminEdit(int $id): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!empty($password)) {
                ...
                $minLen = $role === 'superadmin' ? 12 : 10;
                $errors = validatePassword($password, $minLen, $data['email'] ?? $admin['email'], $data['full_name'] ?? $admin['full_name']);
                if (!empty($errors)) {
                    flash('error', implode('<br>', $errors));
                    redirect('admin/admins/edit/' . $id);
                }
    ...
    public function profilePassword(): void {
        $this->checkAuth();
        $admin = $this->adminModel->find(getCurrentAdminId());
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $minLen = $admin['role'] === 'superadmin' ? 12 : 10;
            $errors = validatePassword($new, $minLen, $admin['email'], $admin['full_name']);
            if (!empty($errors)) {
                flash('error', implode('<br>', $errors));
                redirect('admin/profile/password');
            }
        ...
        require __DIR__ . '/../views/admin/profile_password.php';
    }
